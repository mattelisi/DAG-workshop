...
# Civil engineering doctorates awarded (US; source: NSF) as a function of per-capita mozzarella consumption, 2000--2009.
...
conf_interval <- predict(
  m0,
  newdata = data.frame(mozza_kg = newx),
  interval = "confidence",
  level = 0.975
)
...
plot(
  d$mozza_kg, d$engPhd,
  cex = 1.3, pch = 21, lwd = 2, col = "blue",
  xlab = "Mozzarella consumption [kg/year]",
  ylab = "Civil engineering PhDs [N/year]",
  xlim = c(8.9, 11.2) * 0.453592,
  ylim = c(440, 760)
)
abline(m0, lwd = 1)
...
text(
  4.7, 450,
  bquote(italic(r) ~ "=" ~ .(round(corr_cheese$estimate, 2)) ~ "  " ~ italic(p) ~ "< .0001")
)
...
coord_dag <- list(
  x = c(phds = 1, mozza = -1, third = 0) * 1.5,
...
dag %>%
  ...
  geom_dag_point(colour = "white", size = 36) +
  ...
  theme_dag() +
  coord_equal(xlim = c(-2, 2), ylim = c(-1, 1))

...
::: nonincremental
1.  DAGs (Directed Acyclic Graphs) as nonparametric causal models
...
Link: [mlisi.xyz/DAG-workshop/](https://mlisi.xyz/DAG-workshop/)
...
- For each child, we can define two *potential outcomes*: one if the child receives the intervention, $Y_i^{X=1}$, and one if the child does not, $Y_i^{X=0}$.

- The individual causal effect is the difference between these two outcomes: $$\text{causal effect for child } i = Y_i^{X=1} - Y_i^{X=0}$$


## The fundamental problem of causal inference {background-color="#202A30"}

-   For each child, we observe only one potential outcome: either the child received the treatment or not.\
    The other, *counterfactual*, outcome is fundamentally *unobservable*.

-   The problem is *fundamental* because it can never be solved from data alone; we need **assumptions**.
...
-   **Confounding** occurs when _non-causal_ paths create associations between $X$ and $Y$.

-   A non-causal path is any path connecting $X$ and $Y$ that includes an arrow pointing into $X$. (It is non-causal because changing $X$ does not affect $Y$ through that path.)
...
- More generally, to estimate the causal effect of $X$ on $Y$, we need to **block all non-causal paths** between them.

- In DAG terms, this means finding a set of variables that **d-separates** $X$ and $Y$ along the backdoor paths, without blocking the causal path from $X$ to $Y$.

- Matching and covariate adjustment are two ways of conditioning on variables to block non-causal paths.

 
 
- In practice, some confounders may be *unmeasured*, even if they are represented in the DAG.

- In that case, we need to look for an alternative set of variables that can still block the backdoor paths.
...
2. Focus on the **backdoor paths**: paths that begin with an arrow into $X$.

3. Use the rules of **d-separation** to determine which of these paths are open.

4. Condition on variables that close all open backdoor paths, while leaving the causal path from $X$ to $Y$ unblocked.

::: fragment
&nbsp;

[**If no such set exists, the causal effect is not identifiable from the available data**]{.underline}
:::
...
-   In practice, this often involves a *"two-step" regression*
    -   *Step 1*: $X \sim I + Z$
...
_Conditional instrumental variable:_ $I$ is valid as instrumental variable only if we condition on $Z$
...
- Working individually or in small groups, **draw a DAG** for a study you have done, or would like to do.

- **Mark one causal effect of interest** with an arrow \(X \rightarrow Y\).

- **Upload your DAG**:
  - export it from **dagitty.net**, or
  - draw it on paper, take a photo, and upload the image.

<!-- [Link for uploading](https://www.dropbox.com/request/Po0sjCZMkyh84PncEXXF) -->

[Upload here or scan the QR code](https://padlet.com/matteolisi3/dag-exercise-upload-your-dag-image-6j9elh4j3h9wn0gm)

- We will look at some submissions together and discuss:
  - What assumptions does the DAG imply?
  - Which variables, if any, should we adjust for?
...
![](img/padlet-code.png){fig-align="center"}
...
-   DAGs help us think [explicitly]{.underline} about causal pathways among variables of interest.
...
-   More broadly, graphical causal models provide a rigorous and transparent way to state and scrutinise causal assumptions.
...
-   Attempting to draw plausible a DAG can be a highly uncomfortable experience, as it forces us to confront how uncertain we are about the system we are studying.

-   [DAGs make our assumptions transparent and easier to critique.]{.underline}

-   Unless we can intervene directly (e.g. in an RCT), a single study rarely provides conclusive evidence about causality.

-   Building a convincing causal DAG usually requires broad, converging evidence from multiple studies and designs.
...
- DAGs are deliberately simple models: they clarify assumptions, but they cannot replace fully mechanistic or dynamic models.

- Acyclic graphs cannot fully represent systems with feedback loops (e.g. the brain or the economy).

- Such systems may exhibit forms of complexity, such as sensitivity to initial conditions, that DAGs are not designed to capture.
